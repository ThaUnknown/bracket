...
try {
  ...
  if (idleDetector) {
    idleDetector.start({ threshold: 60_000 }).catch(console.warn)
  }
} catch (error) {
  console.warn(error)
}
...
export const visibilityState = readable(document.visibilityState, set => {
  set(document.visibilityState)
  const ctrl = new AbortController()
  document.addEventListener('visibilitychange', () => set(document.visibilityState), ctrl)
  return () => ctrl.abort()
})
